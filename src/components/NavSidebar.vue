...
<template>
  ...
  >
    ...
    >
      <component
        :is="item.icon"
        class="pointer-events-none h-6 w-6 text-gray-700 dark:text-neutral-300"
      />
    </button>
    <div class="absolute bottom-3 flex flex-col space-y-2">
      <button
        class="flex h-10 w-10 items-center justify-center rounded-md bg-white dark:bg-neutral-800"
        @click="openGitHub"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 480 512"
          class="h-5 w-5 fill-gray-700 dark:fill-neutral-300"
        >
          <path
      ...
      >
        <component
          ...
          class="h-5 w-5 fill-gray-700 text-gray-700 dark:fill-neutral-300 dark:text-neutral-300"
        />
      </button>
    </div>
  </div>
...
const handleTab = (tab) => {
  indicatorRef.value.style.top = `${
    tab.target.getBoundingClientRect().top
    - tab.target.parentElement.getBoundingClientRect().top
  }px`;
...
const moveButton = () => {
  indicatorRef.value.style.width = `${
    tabRefs[0].getBoundingClientRect().width
  }px`;
  indicatorRef.value.style.top = `${
    tabRefs[0].getBoundingClientRect().top
    - tabRefs[0].parentElement.getBoundingClientRect().top
  }px`;
};
